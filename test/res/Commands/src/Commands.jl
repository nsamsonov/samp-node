...
@Hooks.hook OnPlayerCommandText function(playerid::Integer, commandString::String)
...
macro jcmd(x::Expr)
   ...
   @assert get(signature.args, 1, nothing) isa Symbol "missing command name"
...
@jcmd function jcmdtestintpoo(playerid::PlayerId)
   println("inside jcmdtestint $(playerid) $(num)")
end

@jcmd function jcmdtestintfoo()
   println("inside jcmdtestint $(playerid) $(num)")
end
